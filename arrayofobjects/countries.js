...
//india is exist or not
var Exist=countries.some(c=>c.name=="India");
console.log(Exist);

//is there any country with morethan 2 languages
var isExist=countries.some(c=>c.language.length>2);
console.log(isExist);

//isthere any country with morethan three borders
var isBorder=countries.some(c=>c.borders.length>3);
...
{
    let cName=c.name;
    let cCurrency=c.currency;
    if(cCurrency in sameCurrency)
    {
        sameCurrency[cCurrency].push(cName);
    }
    else{
        sameCurrency[cCurrency]=[cName];
    }
}
console.log(sameCurrency);

// 3. Sort all countries in alphabetical order (by country name)
// var countryOrder=countries.sort((c1,c2)=>c1.name.localeCompare(c2.name));
...
var highPopulation=countries.reduce((c1,c2)=>c1.population>c2.population?c1:c2);
console.log(highPopulation);


// 6. Categorize (group) countries based on language.
// var sortedLang=countries.sort((c1,c2)=>c1.language-c2.language);
// console.log(sortedLang);


// 7. List countries that have more than one language.
// var isExisted=countries.filter(c=>c.language.length>1);
// console.log(isExisted);

// 8. Find the average population of all countries.
var avg=countries.reduce((s,p)=>(s+p.population/p.population.length),0);
console.log(avg);
...
var listEur=countries.filter(c=>c.currency.toLowerCase()=="eur");
console.log("EUR used countries:",listEur);
...
var sortpopulation=countries.sort((p1,p2)=>p1.population-p2.population);
